...
json_folder_path = "tshark_json//"
json_info = "tshark_list.json"

# pcap 추출
def start():
    input_folder = "D:\\script\\wireshark\\pcaps\\10gb.pcap"   # pcap 파일 모아놓은 폴더 경로
    ...
    with open(f"{json_folder_path}{name_only}", 'w') as json_file:
...
def all_delete():
    ...
    if not os.path.exists(json_folder_path):
        if not os.path.exists(json_info):
            ...
            return "Json_List Not Found"
    else:
        return "Json_Folder Not Found"
    
def delete_json(target_name):
    with open(json_info, "r", encoding="utf-8") as f:
        data = json.load(f)

    target_file_path = os.path.join(os.path.dirname(json_folder_path), target_name)
...
def json_search(target_name):
    target_json = f"{json_folder_path}{target_name}"
