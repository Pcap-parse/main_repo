...
def extract_conv(layer, pcap_file):
    ...
    # 에러 확인
    if tsp.returncode != 0:
        raise Exception(f"layer {layer}: {tsp.stderr}")
    first_timestamp = tsp.stdout.strip()  # 첫 번째 타임스탬프 반환
    
    return result.stdout, first_timestamp
...
def parse_conv(layer, tshark_output, tsp_list):
    ...
    tsp_min = -1
    # 정규표현식으로 데이터 추출
    for output, tsp_output in zip(tshark_output, tsp_list):
        output = " ".join(output) if isinstance(output, list) else output # 첫번째 요소가 문자열이 아니라 리스트 타입이여서 변환 필요.
        tsp_output = float(tsp_output)
        if tsp_min == -1 or tsp_min > tsp_output:
            tsp_min = tsp_output
        ...
        for match in matches:
            ...
            conversation.update({
                ...
                "rel_start": tsp_output+float(match[8]), # 타임스탬프 형식으로 저장
                "duration": tsp_output+float(match[8])+float(match[9]), # 타임스탬프 형식으로 저장
                "stream_id": -1
            })
...
def main():
    # pcap 파일 경로
    pcap_file = r"D:\downloads\wrccdc.2017-03-24.010540000000000.pcap\wrccdc.2017-03-24.010117000000000.pcap"
    # JSON 형식으로 추출된 데이터를 저장할 파일
    output_file = r"D:\downloads\wrccdc.2017-03-24.010540000000000.pcap\wrccdc.2017-03-24.010117000000000.json"
    ...
    for layer in layers:
        ...
        with Pool(processes=os.cpu_count()) as pool:  # CPU 코어 개수만큼 병렬 실행
            results = pool.map(extract_with_layer, pcap_files)        
        for result, tsp in results:
            tshark_output.append(result)
            tsp_list.append(tsp)

        convs, tsp_min = parse_conv(layer, tshark_output, tsp_list)
        convs[layer] = combine_packets(layer, convs[layer], tsp_min)
